...
* 그래프 표현 방법
    - 1. 인접행렬(adjacency matrix): 인접성을 **행렬(2차원 배열,리스트)**로 표현
        - 장점: 에지의 검색, 삽입, 삭제 연산을 상수 시간 내에 수행할 수 있다.
        - 단점: 1) 메모리를 많이 차지한다. => O(n^2)  
                2)'u에 인접한 모든 노드 v'에 대해 살펴볼 때, 인접한 노드가 존재하지 않더라도 일단 모든 노드를 체크해야 한다.
    - 2. 인접리스트(adjacency list): 각 정점에 인접한 에지만을 **연결리스트**로 표현
        - 장점: 1)메모리를 적게 차지한다. => O(n+m) # n = 노드 개수 , m = 에지 개수  
                2)'u에 인접한 모든 노드 v'에 대해 살펴볼 때, 인접한 노드의 수만큼만 정확하게 for문을 돌면서 체크한다.
        - 단점: 에지의 검색, 삽입, 삭제 연산을 할 때 O(n)시간이 걸린다.  
